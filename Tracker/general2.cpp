...
void Menu2() { // Отдельное меню для калькулятора результатов
...
void calculator(double distans, double time, double temp, int select, int repeat) {
	system("cls");
	do {
		Menu2();
		cin >> select;
...
int main() {
	...
	ofstream send; // Создаем обьект класса для работы с файлами
	send.open("Mytren.txt", ofstream::app); // Создаем папку для сохранения тренировок с последующим добавлением
	...
	do {
		cout << "Вас приветствует трекер СПОРТ, выберите желаемый режим работы: 1 - трекер, 3 - калькулятор ";
		cin >> select;
		if (select == 1) {
			...
			cout << "Запустить секундомер? - 1, Запустить таймер - 2, Открыть калькуляторы результатов - 3" << endl;
			cin >> go; // Запрашиваем от пользователя разрешение что запустить
			select = go; // Записываем выбранный код тренировки
		}
		else if (select == 3) {
			calculator(distans, time, temp, select, repeat);
		}
		Menu(); // Выводим меню тренировок для пользователя
		cin >> type; // Запрашиваем название тренировки
		name2 = Table(type, name); // Определяем название тренировки и присваиваем новой переменной
		...
		if (select == 1) { // В зависимости от типа замера времени - выводим результат
			send << "Результат занятия: " << step.name << "\t" << step.hour << " часы\t" << step.minute << " минуты\t" << step.second << " секунды" << endl; // Сохраняем занятие в файл
		...
		cout << "Назад в меню? 1 - да, 0 - нет, 2 - выгрузить мои тренировки" << endl; // Спрашиваем желание запустить новую тренировку
		cin >> repeat;
		...
		case 2:
			...
			cout << "Назад в меню? 1 - да, 0 - нет" << endl; // Спрашиваем желание запустить новую тренировку
			cin >> repeat;
		}
	} while (repeat == 1);
